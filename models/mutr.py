...
class MUTR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if not self.training:
            ...
            return out, mti_hs_box
        
        return out


    def inference_long_term(self, mti_hs_box_list, t, b=1):

        mti_hs_box = torch.concat(mti_hs_box_list,dim=1)
        mti_hs = self.mti(mti_hs_box)
        hs = mti_hs.repeat(1, t, 1, 1)

        outputs_classes = []    # [l, b, q, c]
        for lvl in range(hs.shape[0]):
            outputs_class = self.class_embed[lvl](hs[lvl])
            outputs_classes.append(outputs_class)
        outputs_class = torch.stack(outputs_classes)
        ...
        return outputs_class[-1][0] # [batch_size, time, num_queries_per_frame, num_classes]
