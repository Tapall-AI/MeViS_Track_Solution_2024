...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        assert name in ['swin_t_p4w7', 'swin_l_p4w7']
        cfgs = configs[name]
...
def build_swin_fpn_backbone(args):
    position_embedding = build_position_encoding(args)
    backbone = Backbone(args.backbone, args.glip_checkpoint, args.use_checkpoint)
    model = Joiner(backbone, position_embedding)
    return model


configs = {
    'swin_t_p4w7': dict(patch_size=4,
            in_chans=3,
            embed_dim=96,
            depths=[2, 2, 6, 2],
            num_heads=[3, 6, 12, 24],
